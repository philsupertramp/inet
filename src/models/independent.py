...
class IndependentModel:
    ...
    def evaluate_model(self, validation_set, preprocessing_method, render_samples = False):
        ...
        predicted_labels_argmax = tf.argmax(predicted_labels_argmax, axis=1)
        ...
        if render_samples:
            ...
            plt.savefig(f'independent-model-confusion.eps', bbox_inches='tight', pad_inches=0)
            plt.savefig(f'independent-model-confusion.png', bbox_inches='tight', pad_inches=0)
            plot_prediction_samples(predicted_bbs, predicted_labels, validation_values)
            plt.savefig(f'independent-model-predictions.eps', bbox_inches='tight', pad_inches=0)
            plt.savefig(f'independent-model-predictions.png', bbox_inches='tight', pad_inches=0)
